classdef Log4jConfigurator
    ...
    % Examples:
    %
    % yaml4mat.logger.Log4jConfigurator.configureBasicConsoleLogging
    %
    % yaml4mat.logger.Log4jConfigurator.setLevels({'root','DEBUG'});
    %
    % yaml4mat.logger.Log4jConfigurator.setLevels({
    %     'root'    'INFO'
    %     'net.apjanke.logger.swing'  'DEBUG'
    %     });
    %
    % yaml4mat.logger.Log4jConfigurator.prettyPrintLogConfiguration
    %
    % % Display fully-qualified class/category names in the log output:
    % yaml4mat.logger.Log4jConfigurator.setRootAppenderPattern(...
    %    ['%d{HH:mm:ss.SSS} %p %c - %m' sprintf('\n')]);
    %
    % % Bring up the configuration GUI
    % yaml4mat.logger.Log4jConfigurator.showGui
    ...
    methods (Static)
        ...
        function setLevels(levels)
            ...
            %
            % yaml4mat.logger.Log4jConfigurator.setLevels(levels)
            %
            ...
            for i = 1:size(levels, 1)
                ...
                level = yaml4mat.logger.Log4jConfigurator.getLog4jLevel(levelName);
