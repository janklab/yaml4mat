classdef Logger
  ...
  % yaml4mat.logger logging happens.
  %
  ...
  % arguments.
  %    yaml4mat.logger.info(format, varargin)
  ...
  % message.
  %    yaml4mat.logger.warn(exception, format, varargin)
  %
  % See also:
  % yaml4mat.logger.error
  % yaml4mat.logger.warn
  % yaml4mat.logger.info
  % yaml4mat.logger.debug
  % yaml4mat.logger.trace
  %
  % Examples:
  %
  % log = yaml4mat.logger.Logger.getLogger('foo.bar.FooBar');
  ...
  methods (Static)
    function out = getLogger(identifier)
      ...
      out = yaml4mat.logger.Logger(jLogger);
    end
  end
